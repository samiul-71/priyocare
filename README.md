...
| `EMAIL_PROVIDER_CONFIGURED` | staff self-service forgot-password | Set to `1` to switch on email. When `1`, `SMTP_HOST` / `SMTP_USER` / `SMTP_PASS` / `SMTP_FROM` become **required at boot** — a half-configured sender is worse than none. Off → `/office/forgot-password` points at admin reset instead, and the console sender throws in production. |
| `SMTP_HOST` `SMTP_PORT` `SMTP_USER` `SMTP_PASS` `SMTP_FROM` `SMTP_SECURE` | the email sender (`lib/server/email/mailer.ts`, nodemailer) | SMTP transport. Interim provider is Mailtrap (`sandbox.smtp.mailtrap.io:2525`, `SMTP_SECURE=false` — 587/2525 start plaintext and STARTTLS-upgrade; only 465 sets `SMTP_SECURE=true`). `SMTP_PORT` defaults to 587, `SMTP_FROM` to `PriyoCare <no-reply@priyocare.app>`. Mailtrap **Sandbox** captures mail; use a real sender host for production delivery. |
...
npm run db:create-staff -- --email x@y.z --password '…' [--name N] [--role ops|admin]
                    ...
                    # self-registration. Min 6 chars; they must change it at first sign-in.
